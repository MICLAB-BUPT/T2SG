...
from ...utils import TRAFFIC_ELEMENT_ATTRIBUTE, NODE_ATTRIBUTE
...
def _mAP_topology_lclc(gts, preds, distance_thresholds):
    ...
    for distance_threshold in distance_thresholds:
        ...
        for token in gts.keys():
            ...
            small_each_ferchet_acc.append(_AP_directerd(gts=gts_topology_lclc, preds=preds_topology_lclc))
            acc.append(_AP_directerd(gts=gts_topology_lclc, preds=preds_topology_lclc))
        each_ferchet_acc.append(np.hstack(small_each_ferchet_acc).mean())
    # import pdb; pdb.set_trace()
    print(f'each_ferchet_acc: {each_ferchet_acc}')
    if len(acc) == 0:
        return np.float32(0)
    return np.hstack(acc).mean()
def _mAP_topology_lnln(gts, preds, distance_thresholds):
    r"""
    Calculate mAP on topology among lane centerlines.
    ...
    for distance_threshold in distance_thresholds:
        for token in gts.keys():
            preds_topology_lclc_unmatched = preds[token]['topology_lnln']

            idx_match_gt = preds[token][f'lane_node_{distance_threshold}_idx_match_gt']
            gt_pred = {m: i for i, m in enumerate(idx_match_gt) if not np.isnan(m)}

            gts_topology_lclc = gts[token]['topology_lnln']
            if 0 in gts_topology_lclc.shape:
                continue
...
def evaluate(ground_truth, predictions, verbose=True):
    ...
    for token in ground_truth.keys():
        ...
        if predictions is None:
            ...
            preds[token] = predictions[token]['predictions']
             #若没有预测结果，就直接将置信度为0
            if preds[token]['traffic_element'] is None:
                preds[token]['traffic_element']  = gts[token]['traffic_element']
                for i, _ in enumerate(preds[token]['lane_centerline']):
                    preds[token]['lane_centerline'][i]['confidence'] = np.float32(0)
            # if preds[token]['lane_node'] is None:
            #     preds[token]['lane_node']  = gts[token]['lane_node']
            #     for i, _ in enumerate(preds[token]['lane_centerline']):
            #         preds[token]['lane_centerline'][i]['confidence'] = np.float32(0)
    ...
    for token in tqdm(gts.keys(), desc='calculating distances:', ncols=80, disable=not verbose):
        ...
        #如果直接就是小于3.0，就拿掉，不参与计算了
        # node_mask = pairwise(
        #     [gt for gt in gts[token]['lane_node']],
        #     [pred['points'] for pred in preds[token]['lane_node']],
        #     chamfer_distance,
        #     relax=True,
        # ) < THRESHOLDS_FRECHET[-1]
        ...
        # distance_matrixs['node_frechet'][token] = pairwise(
        #     [gt for gt in gts[token]['lane_node']],
        #     [pred['points'] for pred in preds[token]['lane_node']],
        #     frechet_distance,
        #     mask=node_mask,
    ...
    # gts = io.pickle_load('topoformer_gt.pkl')
    # preds = io.pickle_load('topoformer_pred.pkl')
    # distance_matrixs = io.pickle_load('topoformer_distance_matrixs.pkl')
    # 保存distance_matrixs
    # io.pickle_dump('baseline_distance_matrixs.pkl', distance_matrixs)
    # io.pickle_dump('baseline_gt', gts)
    # io.pickle_dump('baseline_pred', preds)
    """
        evaluate
    """
    ...
    metrics['OpenLane-V2 Score']['DET_l_each'] = np.hstack([_mAP_over_threshold(
        gts=gts, 
        preds=preds, 
    ...
    ) for idx in NODE_ATTRIBUTE.values()])
    ...
    # import pdb; pdb.set_trace()
    print("metrics['OpenLane-V2 Score']['DET_l_each']", metrics['OpenLane-V2 Score']['DET_l_each'])
    print("metrics['OpenLane-V2 Score']['DET_l']", metrics['OpenLane-V2 Score']['DET_l'])
    metrics['OpenLane-V2 Score']['DET_l'] = _mAP_over_threshold(
        gts=gts, 
        preds=preds, 
        distance_matrixs=distance_matrixs['frechet'], 
    ...
    # metrics['OpenLane-V2 Score']['DET_ln'] = np.hstack()_mAP_over_threshold(
    #     gts=gts, 
    #     preds=preds, 
    #     distance_matrixs=distance_matrixs['node_frechet'], 
    #     distance_thresholds=THRESHOLDS_FRECHET,
    #     object_type='lane_node',
    #     filter=lambda x: x['attribute'] == idx,
    #     inject=True, # save tp for eval on graph
    # )for idx in NODE_ATTRIBUTE.values()).mean()
    # metrics['OpenLane-V2 Score']['DET_ln'] = np.hstack([_mAP_over_threshold(
    #     gts=gts, 
    #     preds=preds, 
    #     distance_matrixs=distance_matrixs['node_frechet'], 
    ...
    metrics['OpenLane-V2 Score']['DET_t_each'] = [_mAP_over_threshold(
        gts=gts, 
        preds=preds, 
        distance_matrixs=distance_matrixs['iou'], 
        distance_thresholds=THRESHOLDS_IOU, 
    ...
    print("metrics['OpenLane-V2 Score']['DET_t_each']", metrics['OpenLane-V2 Score']['DET_t_each'])
    print("metrics['OpenLane-V2 Score']['DET_t']", metrics['OpenLane-V2 Score']['DET_t'])
    _mAP_over_threshold(
        gts=gts, 
        preds=preds, 
        distance_matrixs=distance_matrixs['iou'], 
