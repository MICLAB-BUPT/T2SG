...
def _draw_lane_centerline(image, lane_centerline, with_attribute):
    ...
    for idx, color in enumerate(colors):
        for i in range(len(points) - 1):
            ...
            # cv2.arrowedLine(image, (y1, x1), (y2, x2), color, THICKNESS, cv2.LINE_AA, 0, 0.3)
            cv2.arrowedLine(image, pt1=(y1, x1), pt2=(y2, x2), color=color, thickness=THICKNESS, line_type=cv2.LINE_AA, tipLength=0.3)
