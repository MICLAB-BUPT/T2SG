...
def _draw_lane_centerline(image, lane_centerline, intrinsic, extrinsic, with_attribute):
    ...
    if with_attribute and len(set(lane_centerline['attribute']) - set([0])):
        colors = [COLOR_DICT[a] for a in set(lane_centerline['attribute']) - set([0])]
    else:
...
def _draw_topology(image, topology, intrinsic, extrinsic):
    ...
    cv2.polylines(image, curve, isClosed=False, color=color, thickness=5, lineType=cv2.LINE_AA)
