_base_ = []
custom_imports = dict(imports=['projects.bevformer', 'projects.topoformer'])
...
data_root = '/home/xcsz/OpenLane-V2/data/OpenLane-V2/'
...
bev_h_ = 100
bev_w_ = 200
is_pretrain_detectors = True
model = dict(
    type='TopoNet',
    is_pretrain_detectors=is_pretrain_detectors,
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
    ...
    lclc_head=dict(
            ...
            use_topologic=False,
            distance_mapping_function='topologic',
            alpha=0.2,
            lambd=2.0,
            lamda_sim=1.0,
            lamda_geo=1.0,
    ...
    # lclc_head=dict(
    #     type='CustomDeformableDetrMLPPredictionHead',
    #     input_dim=2*_dim_,
    #     hidden_dim=_dim_, 
    #     output_dim=1, 
    #     num_layers=3,
    ...
    train_cfg=dict(
        ...
        lane=dict(
            assigner=dict(
                ...
                pc_range=point_cloud_range))),
    # init_cfg=dict(type='Pretrained', checkpoint='/root/autodl-tmp/aaai2025/work_dirs/toponet/exp_44/epoch_20.pth')
    )
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations3DLane',
         ...
         with_bbox=True, with_label=True, with_lane_lcte_adj=True, with_lane_lcte_adj_only_ls=True),
    ...
    dict(type='CustomCollect3D', keys=[
        ...
        'gt_bboxes', 'gt_labels', 'gt_lane_lcte_adj','gt_lane_lcte_adj_only_ls'])
]
val_pipeline = [
    dict(type='CustomLoadMultiViewImageFromFiles', to_float32=True),
    dict(type='LoadAnnotations3DLane',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_train_3d_lane_label_lcte_only_ls.pkl',
    ...
    val=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'data_dict_subset_A_val_3d_lane_label_lcte_only_ls.pkl',
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            # 'lclc_head': dict(lr_mult=5),
        }),
    weight_decay=0.01)

optimizer_config = dict(grad_clip=dict(max_norm=35, norm_type=2),detect_anomalous_params=True)
# learning policy
lr_config = dict(
