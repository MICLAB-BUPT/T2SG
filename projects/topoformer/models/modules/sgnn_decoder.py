...
class SGNNDecoder(TransformerLayerSequence):

    def __init__(self, *args, return_intermediate=False, is_pretrain_detectors=False,  **kwargs):
        super(SGNNDecoder, self).__init__(*args, **kwargs)
    ...
    def forward(self,
        ...
        prev_lclc_adj = torch.zeros((query.size(1), num_query, num_query),
                                    dtype=query.dtype, device=query.device)
        prev_lcte_adj = torch.zeros((query.size(1), num_query, int(num_query/2)),
                                    dtype=query.dtype, device=query.device)
        for lid, layer in enumerate(self.layers):
            ...
            lclc_rel_out = prev_lclc_adj.detach()
            if self.is_pretrain_detectors:
                lcte_rel_out = prev_lcte_adj.detach()
            else:
