...
class TopoFormer(MVXTwoStageDetector):

    def __init__(self,
                 is_pretrain_detectors=False,
        ...
        super(TopoFormer, self).__init__(**kwargs)
    ...
    def forward_train(self,
        ...
        if self.is_pretrain_detectors:
            ...
            #若不止是训练检测器，那就会把lclc也加进来
            outs = self.lclc_head(outs)
        # else:
        #     outs = self.pts_bbox_head(img_feats, bev_feats, img_metas, te_feats, te_cls_scores)
        #     outs = self.lclc_head(outs)
            #否则，还是会计算损失
        # for name, param in self.lclc_head.named_parameters():
        #     if param.requires_grad:
        #         print(f'{name} grad: {param.grad}')
                # import pdb; pdb.set_trace()
        loss_inputs = [outs, gt_lanes_3d, gt_lane_labels_3d, gt_lane_adj, gt_lane_lcte_adj,gt_lane_lcte_adj_only_ls,te_assign_result]
        # if self.is_pretrain_detectors:
        lane_losses,lcte_losses, lclc_preds, lclc_targets = self.pts_bbox_head.loss(*loss_inputs, img_metas=img_metas)
        if self.is_pretrain_detectors:
            pass
        else:
            lclc_loss = self.lclc_head.loss(lclc_preds,lclc_targets)
            losses['lclc_head.lclc_loss'] = lclc_loss
            for loss in lcte_losses:
                losses['lane_head.' + loss] = lcte_losses[loss]
        #其中包含lcte的loss
    ...
    def simple_test_pts(self, x, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        if self.is_pretrain_detectors:
            ...
            outs = self.lclc_head(outs)
            lclc_results = self.lclc_head.get_lclc(outs)
        # bbox_results = None 
    ...
    def simple_test(self, img_metas, img=None, prev_bev=None, rescale=False):
        ...
        for result_dict, bbox, lane, lclc, lcte in zip(results_list, bbox_results, lane_results, lclc_results, lcte_results):
            ...
            result_dict['lcte_results_only_ls'] = lcte
        # for result_dict, lane, lclc, lcte in zip(results_list, lane_results, lclc_results, lcte_results):
        #     result_dict['bbox_results'] = None
        #     result_dict['lane_results'] = lane
        #     result_dict['lclc_results'] = None
        #     result_dict['lcte_results'] = None
