...
class TopoFormerHead(AnchorFreeHead):

    def __init__(self,
                 is_pretrain_detectors,
                 num_classes,
                 in_channels,
                 num_query=100,
                 transformer=None,
        ...
        self.is_pretrain_detectors = is_pretrain_detectors
        #
        if self.is_pretrain_detectors:
            lclc_head = None
            lcte_head = None
        ...
        self.use_pre_train_derectors = use_pre_train_derectors
        self._init_layers()
    ...
    def _init_layers(self):
        ...
        if not self.is_pretrain_detectors:
        # lclc_head = build_head(self.lclc_cfg)
        ...
        # self.proj_q = _get_clones(proj_q, num_pred)
        # self.proj_k = _get_clones(proj_k, num_pred)
        if not self.is_pretrain_detectors:
    ...
    def forward(self, mlvl_feats, bev_feats, img_metas, te_feats=None, te_cls_scores=None):
        ...
        outputs = self.transformer(
            ...
            lcte_branches=self.lcte_branches if not self.is_pretrain_detectors else None,
            te_feats=te_feats,
        ...
        lclc_rel_out = lclc_rel_out.unsqueeze(-1)
        outs = {
    ...
    def loss_single(self,
        ...
        if not self.is_pretrain_detectors:
            loss_lcte = self.lcte_branches[layer_index].loss(lcte_preds, gt_lane_lcte_adj_list_only_ls, assign_result, te_assign_result)['loss_rel']
        else:
            loss_lcte = torch.tensor(0.0).to(lclc_preds.device)
        if digit_version(TORCH_VERSION) >= digit_version('1.8'):
            loss_cls = torch.nan_to_num(loss_cls)
            loss_bbox = torch.nan_to_num(loss_bbox)
        return loss_cls, loss_bbox, lclc_preds, lclc_targets, loss_lcte
    ...
    def loss(self,
             ...
             gt_lane_adj,
             gt_lane_lcte_adj,
             gt_lane_lcte_adj_only_ls,
             te_assign_results=None,
             gt_bboxes_ignore=None,
             img_metas=None):
        ...
        all_gt_lane_lcte_adj_list_only_ls = [gt_lane_lcte_adj_only_ls for _ in range(num_dec_layers)]
        ...
        # import pdb; pdb.set_trace()
        losses_cls, losses_bbox,lclc_preds,lclc_targets,losses_lcte = multi_apply(
            self.loss_single, all_cls_scores, all_lanes_preds, all_lclc_preds, all_lcte_preds,te_assign_results, 
            all_gt_lanes_list, all_gt_labels_list, all_gt_lane_adj_list, all_gt_lane_lcte_adj_list, all_gt_lane_lcte_adj_list_only_ls,layer_index)
        loss_dict = dict()
        lcte_dict=dict()
        ...
        lcte_dict['loss_lcte_rel'] = losses_lcte[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_lcte_i in zip(
            losses_cls[:-1], losses_bbox[:-1], losses_lcte[:-1]):
            ...
            lcte_dict[f'd{num_dec_layer}.loss_lcte_rel'] = loss_lcte_i
            num_dec_layer += 1
        return loss_dict,lcte_dict,lclc_preds,lclc_targets
