...
#     return image
def show_bev_results(gt_lane, pred_lane, gt_lclc=None, pred_lclc=None, only=None, map_size=[-55, 55, -30, 30], scale=10, gt_lane_attribute=None, pred_lane_attribute=None):
    image = np.ones((int(scale*(map_size[1]-map_size[0])), int(scale*(map_size[3] - map_size[2])), 3), dtype=np.uint8) * 255  # 将图像背景设置为白色

    GT_COLOR = (0, 128, 0)  # 绿色
    PRED_COLOR = (255, 0, 0)  # 蓝色
    COLOR_DICT = {
    0:  (0, 0, 255),
    1:  (255, 0, 0),
    2:  (0, 255, 0),
    ...
    if only is None or only == 'gt':
        ...
        if gt_lane_attribute is not None:
            for lane, attribute in zip(gt_lane,gt_lane_attribute):
                draw_coor = (scale * (-lane[:, :2] + np.array([map_size[1], map_size[3]]))).astype(int)
                if attribute == 0:
                    color = GT_COLOR
                else:
                    color = COLOR_DICT[attribute]
                image = cv2.polylines(image, [draw_coor[:, [1,0]]], False, GT_COLOR, max(round(scale * 0.2),10))
                image = cv2.circle(image, (draw_coor[0, 1], draw_coor[0, 0]), max(2, round(scale * 0.5)), GT_COLOR, -1)
                image = cv2.circle(image, (draw_coor[-1, 1], draw_coor[-1, 0]), max(2, round(scale * 0.5)) , GT_COLOR, -1)
        else:
            for lane in gt_lane:
    ...
    if only is None or only == 'pred':
        if pred_lane_attribute is not None:
            # print(len(pred_lane))
            flag = 0
            for lane,attribute in zip(pred_lane,pred_lane_attribute):
                flag += 1
                draw_coor = (scale * (-lane[:, :2] + np.array([map_size[1], map_size[3]]))).astype(int)
                if attribute == 0:
                    color = PRED_COLOR
                    ...
                    color = COLOR_DICT[attribute]
                image = cv2.polylines(image, [draw_coor[:, [1,0]]], False, PRED_COLOR, max(round(scale * 0.2), 10))
                image = cv2.circle(image, (draw_coor[0, 1], draw_coor[0, 0]), max(2, round(scale * 0.5)), PRED_COLOR, -1)
                image = cv2.circle(image, (draw_coor[-1, 1], draw_coor[-1, 0]), max(2, round(scale * 0.5)) , PRED_COLOR, -1)
        else:
            for lane in pred_lane:
