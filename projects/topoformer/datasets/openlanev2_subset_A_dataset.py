...
class OpenLaneV2_subset_A_Dataset(Custom3DDataset):
    ...
    LANE_ATTR_CLASSES = ('lane','go_straight', 'turn_left', 'turn_right',
                       'no_left_turn', 'no_right_turn', 'u_turn', 'no_u_turn',
                       'slight_left', 'slight_right')
    MAP_CHANGE_LOGS = [
    ...
    def get_ann_info(self, index):
        ...
        gt_lane_labels_3d = np.array([lane['attribute'] for lane in ann_info['lane_centerline']], dtype=np.int64)
    ...
    def format_results(self, results, jsonfile_prefix=None, **kwargs):
        ...
        for idx, result in enumerate(results):
            ...
            # if result['lcte_results_only_ls'] is not None:
            #     pred_info['topology_lcte_only_ls'] = result['lcte_results_only_ls'].astype(np.float32)[valid_indices][:, te_valid_indices]
            # if result['lnln_results'] is not None:
            #     pred_info['topology_lnln'] = result['lnln_results'].astype(np.float32)[valid_indices][:, valid_indices]
            # else:
            #     pred_info['topology_lnln'] = np.zeros((len(pred_info['lane_centerline']), len(pred_info['lane_centerline'])), dtype=np.float32)
    ...
    def evaluate(self, results, logger=None, show=False, out_dir=None, **kwargs):
        ...
        #保存这两个dict
        # with open(os.path.join(out_dir, 'gt_dict.pkl'), 'wb') as f:
        #     pickle.dump(gt_dict, f)
        # with open(os.path.join(out_dir, 'exp_49_pred_dict.pkl'), 'wb') as f:
    ...
    def show(self, results, out_dir, score_thr=0.3, show_num=200, **kwargs):
        """Show the results.
        ...
        for idx, result in enumerate(results):
            ...
            gt_lanes = []
            gt_lanes_attribute = np.array([])
            for lane in info['annotation']['lane_centerline']:
                gt_lanes.append(lane['points'])
                attributes = lane['attribute']
                if isinstance(attributes, set) and any(attr > 3 for attr in attributes):
                    gt_lanes_attribute = np.concatenate((gt_lanes_attribute, np.array([attr for attr in attributes if attr > 3])))
                else:
                    gt_lanes_attribute = np.append(gt_lanes_attribute, 0)
            ...
            pred_lanes_attribute = np.array([])
            for lane in pred_result['lane_centerline']:
                attributes = lane['attribute']
                if isinstance(attributes, set) and any(attr > 3 for attr in attributes):
                    pred_lanes_attribute = np.concatenate((pred_lanes_attribute, np.array([attr for attr in attributes if attr > 3])))
                else:
                    pred_lanes_attribute = np.append(pred_lanes_attribute, 0)
            ...
            # output_path = os.path.join(out_dir, f'{info["segment_id"]}/{info["timestamp"]}/surround_raw.jpg')
            ...
            # conn_img_gt = show_bev_results(gt_lanes, pred_lanes, gt_lclc, pred_lclc, only='gt', map_size=[-52, 52, -27, 27], scale=20, gt_lane_attribute=gt_lanes_attribute, pred_lane_attribute=pred_lanes_attribute)
            # conn_img_pred = show_bev_results(gt_lanes, pred_lanes, gt_lclc, pred_lclc, only='pred', map_size=[-52, 52, -27, 27], scale=20,gt_lane_attribute=gt_lanes_attribute, pred_lane_attribute=pred_lanes_attribute)
