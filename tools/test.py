...
def parse_args():
    ...
    parser.add_argument('--show-dir', help='directory where images will be saved')
    parser.add_argument(
...
def main():
    ...
    if rank == 0:
        ...
        if args.format_only:
            pred_dict = dataset.format_results(outputs, **kwargs)
            save_path = os.path.join(args.out_dir, 'formatted_results.pkl')
            with open(save_path, 'wb') as f:
                pickle.dump(pred_dict, f)
            logger.info(f'Saved formatted results to {save_path}')
